...
This is the full, practical guide to running, configuring, extending, and troubleshooting **Star Wars Galaxies Roleplay (SWGRP)**. For a quick feature overview see [README.md](README.md).

## Table of Contents

1. [Overview](#overview)
2. [Installation](#installation)
3. [First Boot & Verifying the Gamemode Loaded](#first-boot)
4. [Server Configuration (ConVars)](#server-configuration)
5. [Controls & Menus](#controls--menus)
6. [Gameplay Guide](#gameplay-guide)
7. [Authoring Content (CSV)](#authoring-content-csv)
8. [Custom Lua Extensions](#custom-lua-extensions)
9. [Map Setup](#map-setup)
10. [Administration](#administration)
11. [Persistence & Database](#persistence--database)
12. [Hook API Reference](#hook-api-reference)
...
4. Set **Gamemode** to **Star Wars Galaxies RP** and start.

### Dedicated server

1. Place the `swgrp` folder in `garrysmod/gamemodes/` on the server.
2. In your server launch command line / `start.bat`, set:
   ```
   +gamemode swgrp +map rp_yourmap
   ```
3. (Recommended) Add `-condebug` so all console output is written to `garrysmod/console.log` for diagnostics.

### Content (models/materials)

Many professions reference custom player models (e.g. stormtrooper, IG-11, Rodian models). Mount the relevant Star Wars content packs/addons so those models exist on **both** the server and clients. If a model is missing, the menu preview falls back to a default Combine/citizen model and players spawn with that fallback — the gamemode still loads fine.
...
Underworld roles (Smuggler, etc.) can acquire **contraband** (spice, illegal weapons, data chips). Carrying it risks detection by Imperial `/scan`. The **Disguise Kit** conceals identity; the **Pocket** lets you carry weapons discreetly (`/pocket`, `/droppocket`).
...
Register jail positions for the map (see [Map Setup](#map-setup)). Without them, SWGRP now uses a random spawn point rather than `Vector(0,0,0)`.
